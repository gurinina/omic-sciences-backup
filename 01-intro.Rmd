# Introduction to genomics analysis methods: RNA-seq

I thought I would start out the intro to genomic analysis methods by using RNA-seq as an example.

So the goal of this module is to learn software, i.e. coding, and statistical & analytic techniques to tackle genomic data.

But it is important not to lose sight of the goals of your experiments.

So some of the key objectives start at the level of experimental design:
...
```{r}
xp_heat_stress <- tibble(expression = rnorm(n = 1000, mean = 4, sd = 0.5),condition = "heat stress")
...
t.test(x = xp_normal_conditions_more_spread$expression, 
       ...
       conf.level = 0.95)
```
So we can see how critically the outcome of the t-test depends on the statistical parameters of the distributions.

### Type I and type II errors
**Type I errors** (aka false positives) occur when the **H0 (null) hypothesis** is rejected wrongly (e.g., the two treatment means are equal) in favor of an alternative hypothesis (e.g., that the two means are different). Type I errors are probably most familiar to researchers and much effort is expended to ensure that Type I errors are minimized.


**Type II errors** (aka false negatives) also are common in hypothesis testing. These errors result from accepting the null hypothesis when in fact a treatment effect exists.
