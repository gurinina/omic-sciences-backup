...
The Rmd for this file can be found [here](https://github.com/gurinina/omic_sciences/blob/main/19-RNAseq-limma.Rmd).
...
Load the save DGEList from DGEList-object once you've gotten the above to run, or downloaded the count matrix manually:
```{r}
x = readRDS("data/limma.RDS")
```
...
Figure 4: Means (x-axis) and variances (y-axis) of each gene are plotted to show the dependence between the two before voom is applied to the data (top panel) and how the trend is removed after voom precision weights are applied to the data (bottom panel).The plot on the top is created within the `voom` function which extracts residual variances from fitting linear models to log-CPM transformed data. Variances are then rescaled to quarter-root variances (or square-root of standard deviations) and plotted against the average log2 count for each gene. The plot on the bottom is created using `plotSA` which plots log2 residual standard deviations against mean log-CPM values. In both plots, each black dot represents a gene. On the top plot, the red curve shows the estimated mean-variance trend used to compute the voom weights. On the bottom plot, the average log2 residual standard deviation estimated by the empirical Bayes algorithm is marked by a horizontal blue line. 
...
Figure 5: Venn diagram showing the number of genes DE in the comparison between basal versus LP only (left), basal versus ML only (right), and the number of genes that are DE in both comparisons (center) The number of genes that are not DE in either comparison are marked in the bottom-right.
...
```
The mean-difference plot generated by the command above is available online (see http://bioinf.wehi.edu.au/folders/limmaWorkflow/glimma-plots/MD-Plot.html).
